...
def merge(output_file: Path, files_to_merge: list[Path]):
    # Read and concatenate each file content with a single newline
    merged_content = (
        "\n".join(file.read_text().strip() for file in files_to_merge) + "\n"
